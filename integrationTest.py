...
def start():
    ...
    def getDrink():
        ...
        ing= entry1.get()
        def getIng():
            result =c.execute("SELECT DRDESC FROM Drinks WHERE DRDRID IN "
                          "(SELECT DTDRID FROM Detail WHERE INGID "
                          "=(SELECT INGID FROM Ingredients WHERE INDESC LIKE ?))", (ing,))
            resultrow = c.fetchall()
            print(resultrow)
            if resultrow is not None:
                ...
                mess= Message(answerScreen, text=resultrow)
                mess.pack()
    ...
    def drinklist():
        ...
        mylist=list()
        showAll = c.execute("SELECT DRDESC FROM Drinks ORDER BY 1")
